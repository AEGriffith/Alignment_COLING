...
# full model
all_words_full <- brm(count_target | trunc(lb=1) ~ count_prime_own + count_prime_partner +
...
# save model
saveRDS(all_words_full, "all_words_full.rds" )

# model with only count_prime_own
all_words_only_cp_own <- brm(count_target | trunc(lb=1) ~ count_prime_own +
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = word_list_data_study, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
...
# save model
saveRDS(all_words_only_cp_own, "all_words_only_cp_own.rds")

# model with only count_prime_partner
all_words_only_cp_partner <- brm(count_target | trunc(lb=1) ~ count_prime_partner +
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = word_list_data_study, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
...
# save model
saveRDS(all_words_only_cp_partner, "all_words_only_cp_partner.rds")

# null model
all_words_null <- brm(count_target | trunc(lb=1) ~
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = word_list_data_study, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
...
# save model
saveRDS(all_words_null, "all_words_null.rds")

# SUMMARY MODELS
# data
all_words_sum = word_list_data_study %>%
...
# full summary model
sum_all_words_full <- brm(count_target | trunc(lb=1) ~ count_prime_own + count_prime_partner +
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = all_words_sum, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
# save full model
saveRDS(sum_all_words_full, "sum_all_words_full.rds")

# summary model with only count_prime_own
sum_all_words_only_cp_own <- brm(count_target | trunc(lb=1) ~ count_prime_own +
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = all_words_sum, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
# save cp_own model
saveRDS(sum_all_words_only_cp_own, "sum_all_words_only_cp_own.rds")

# full summary model
sum_all_words_only_cp_partner <- brm(count_target | trunc(lb=1) ~ count_prime_partner +
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = all_words_sum, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
# save full model
saveRDS(sum_all_words_only_cp_partner, "sum_all_words_only_cp_partner.rds")

# null summary model
sum_all_words_null <- brm(count_target | trunc(lb=1) ~
                (1 | group)+ (1|group_speaker)+ (1|word) + (1|group_turn_id) + (1|term),
                data = all_words_sum, family = poisson(),
                save_pars = save_pars(all = TRUE ), control = list(adapt_delta=0.99, max_treedepth= 25),
                chains = 6, cores = 6, backend = "cmdstanr", threads = threading(12))
# save null model
saveRDS(sum_all_words_null, "sum_all_words_null.rds")
